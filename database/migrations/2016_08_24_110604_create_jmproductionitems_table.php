...
class CreateJmproductionitemsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Module::generate("JMProductionItems", 'jmproductionitems', 'production_item', 'JM',[
        ["production_item",        "item",         "Name",     false, "",  0,  256,    true],
        ["production_item_dis",        "Discription",         "Name",     false, "",  0,  256,    true],
        ]);
		
        $productionItem = Array
        (
            Array('production_item' => 'Spining'),
            Array('production_item' => 'Loom'),
            Array('production_item' => 'SPG')
        );
       
        foreach ($productionItem as $value) {
            $row = new JMProductionItem;
            $row->production_item = $value['production_item'];
            $row->save();
            };
		/*
		Row Format:
    ...
    {
        if (Schema::hasTable('jmproductionitems')) {
            Schema::drop('jmproductionitems');
        }
    }
}
