...
class CreateAccLedgerTypesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Module::generate("Acc_ledger_types", 'acc_ledger_types', 'ledger_type_id', 'ALL',[
            ["ledger_type_id",         "Ledger Type Id",          "Integer",  false,                  0, 0,  5000,   false],
            ["ledger_type",            "Ledger Type",             "String",   false, "",              0, 256,        true],
        ]);
        ...
        foreach ($suppliers as $value) {
            $row = new Acc_ledger_type;
            $row->ledger_type = $value['ledger_type'];
            $row->ledger_type_id = $value['ledger_type_id'];
            $row->save();
            };
    ...
    {
        if (Schema::hasTable('acc_ledger_types')) {
            Schema::drop('acc_ledger_types');
        }
    }
}
